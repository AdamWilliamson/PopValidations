...
namespace PopValidations.Demonstration_Tests.ValidatorDemoTests.Vitally;
...
public class Vitally_DemoTest
{
    ...
    {
        // Arrange
        var validationRunner = ValidationRunnerHelper.BasicRunnerSetup(new Vitally.Validator());

        // Act
        var validationResult = await validationRunner.Validate(
            new Vitally.InputObject(NString: "")
        );
    ...
    {
        // Arrange
        var descriptionRunner = ValidationRunnerHelper.BasicRunnerSetup(new Vitally.Validator());
